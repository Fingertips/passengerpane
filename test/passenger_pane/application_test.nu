...
(describe "Application" `(
  (it "initializes with attributes" (do ()
    ...
    (~ application aliases should be:"assets.test.local")
    (~ application path should be:"/path/to/test")
    (~ application environment should be:true)
...
(describe "An Application" `(
  ...
  (it "converts its attributes to a dictionary" (do ()
    ...
    (~ (attributes objectForKey:"host") should be:"test.local")
    (~ (attributes objectForKey:"aliases") should be:"assets.test.local")
...
(describe "An Application, concering validation" `(
  (it "is invalid without host and path" (do ()
    ...
    (~ (application isValid) should be:false)
  ))
...
(describe "An Application, concerning dirty checking" `(
  (before (do ()
    (set @attributes (NSMutableDictionary dictionary))
    (@attributes setValue:"test.local" forKey:"host")
    (@attributes setValue:"assets.test.local" forKey:"aliases")
  ...
  (it "is marked as dirty when an attribute is changed" (do ()
    (@application setValue:"test2.local" forKey:"host")
    (~ (@application isDirty) should be:true)
  ))
  
  (it "is no longer marked as dirty when the changed attribute value is reverted" (do ()
    (@application setValue:"test2.local" forKey:"host")
    (~ (@application isDirty) should be:true)
